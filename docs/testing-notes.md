...
2. **Integration (H2 in-memory):** repositories + transactional flows against H2 — Flyway applies cleanly; checkout locking; webhook idempotency (insert duplicate key); expiry↔callback race (two threads, one payment); invariant assertions from database.md.
...
- Use H2 in-memory database (Oracle compatibility mode) for integration tests; dialect drift risk is accepted for v1 simplicity.
